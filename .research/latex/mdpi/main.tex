...
全 8 ランは事前登録コミットの子孫にあたる単一コミットで実行され、
指標はすべて外部の評価層 airas-eval が算出した。
...
$\rho_{\mathrm{real}} = \airasval{rho_jacobcov_real}$、
$\rho_{\mathrm{rand}} = \airasval{rho_jacobcov_rand}$、
$\Delta\rho = \airasval{delta_rho_jacobcov}$。
判定基準は $|\Delta\rho| \le 0.10$ であり、実測値はこれを上回った。
...
$\rho_{\mathrm{real}} = \airasval{rho_snip_real}$、
$\rho_{\mathrm{rand}} = \airasval{rho_snip_rand}$、
$\Delta\rho = \airasval{delta_rho_snip}$。
$|\Delta\rho| \le 0.10$ を満たし、判定基準を通過した。
符号が負である点は予測していなかった。すなわち\textbf{乱数入力の方がわずかに良い}。
$\rho_{\mathrm{real}}$ は文献値 \unverified{0.58} に近い。

\paragraph{C3（データ非依存対照）— 支持.}
$\rho_{\mathrm{real}} = \airasval{rho_synflow_real}$、
$\rho_{\mathrm{rand}} = \airasval{rho_synflow_rand}$、
$\Delta\rho = \airasval{delta_rho_synflow}$。
判定基準 $|\Delta\rho| \le 0.001$ を満たす。表~\ref{tab:zc_summary} のとおり
...
$\rho_{\mathrm{rep}} = \airasval{rho_jacobcov_rep}$ を与え、
$\Delta\rho_{\mathrm{seed}} = \airasval{delta_rho_seed}$ にとどまった。
判定基準 $|\Delta\rho_{\mathrm{jacob\_cov}}| \le |\Delta\rho_{\mathrm{seed}}| + 0.05$ に対し、
実測の $|\Delta\rho_{\mathrm{jacob\_cov}}| = \airasval{delta_rho_jacobcov}$ はこれを大きく上回る。
...
$\airasval{score_agreement_jacobcov}$ であり、判定基準 $<0.999$ を満たす。
...
\paragraph{支持指標.}
jacob\_cov について、Kendall $\tau$ は
$\airasval{tau_jacobcov_real} \to \airasval{tau_jacobcov_rand}$
（差 $\airasval{delta_tau_jacobcov}$）、
真の上位 10\% に限定した Spearman $\rho$ は
$\airasval{rho10_jacobcov_real} \to \airasval{rho10_jacobcov_rand}$
（差 $\airasval{delta_rho10_jacobcov}$）、
上位 10\% の一致率は
$\airasval{prec10_jacobcov_real} \to \airasval{prec10_jacobcov_rand}$
であった。いずれも主要指標と同じ向きで、実データ条件が優る。
ただし上位 10\% 系の指標はシード複製ランでも大きく動いており
（表~\ref{tab:zc_summary}）、主要指標ほど安定していない。
この 2 つの差は主要指標ほどの重みでは読めない。
...
本研究で最も高い順位相関を示したのは synflow（$\airasval{rho_synflow_real}$）であり、
これは構成上\textbf{入力データを一度も参照しない}。
jacob\_cov が実データを使って到達する $\airasval{rho_jacobcov_real}$ を上回っている。
...
実入力と乱数入力のスコアは相関 $\airasval{score_agreement_jacobcov}$ で強く結びついており、
それでもなお真の精度との順位相関には無視できない差が出る。
本研究の寄与は、この差に数値と判定基準を与えた点にある。

\paragraph{判定基準の際どさについて.}
C1 の実測値 $\airasval{delta_rho_jacobcov}$ は判定基準 $0.10$ をわずかに超えたにすぎず、
基準が $0.15$ であれば結論は逆になっていた。この際どさは正直に記録されるべきである。
ただし C4 はその解釈を助ける。シードのみを変えた複製ランの
$\Delta\rho_{\mathrm{seed}} = \airasval{delta_rho_seed}$ と比べると、
...
順位品質に効く。ただしその差は本探索空間では $\Delta\rho \approx \airasval{delta_rho_jacobcov}$ であり、
プロキシの選択（synflow と snip の差は主要指標で
$\airasval{rho_synflow_real}$ 対 $\airasval{rho_snip_real}$）の方が大きい。
